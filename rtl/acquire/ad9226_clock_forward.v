...
module ad9226_clock_forward (
    ...
    wire mode_sync;
    (* KEEP = "TRUE" *) reg mode_latched;
    xpm_cdc_single #(.DEST_SYNC_FF(2), .SRC_INPUT_REG(0)) u_mode_cdc
        (.src_clk(1'b0), .src_in(interleave_enable), .dest_clk(clk_adc_65m), .dest_out(mode_sync));
    always @(posedge clk_adc_65m)
        if (reset) mode_latched <= 1'b0;
        else mode_latched <= mode_sync;
    assign interleave_active = mode_latched;
    // SAME_EDGE 同拍锁存 D1/D2，切换只延长半周期，不产生窄脉冲。
    // 每个输出管脚使用独立 ODDR，确保时钟通过专用 OLOGIC 转发。
    // 注意：两个 ODDR 共用同一时钟源，双通道同相，交织时 B 反相。
    ODDR #(
    ...
    ODDR #(
        ...
        .CE (1'b1),        // 恒使能
        .D1 (!mode_latched),        // 同相 1，交织 0
        .D2 (mode_latched),        // 同相 0，交织 1
        .R  (reset),       // 同步复位
        .S  (1'b0)         // 置位不用
    );
