...
module tb_signal_processing_m6;
    ...
    // 参考文件含连续两窗；本测试的计算停顿使第二窗未能收满。
    reg [367:0] measurement_expected [0:1];
    integer input_index;
    ...
    initial begin
        ...
        if (bucket_size != 32'd32 || measurement_window_samples != 32'd512 ||
            frame_interval_samples != 32'd512) begin
            $fatal(1, "M6 config mismatch K/window/interval=%0d/%0d/%0d",
        ...
        timeout = 0;
        // 最后一桶的输出晚于输入；同时等待包络和测量完成，不补送伪样本。
        while (((envelope_index < ENVELOPE_COUNT) ||
                (measurement_index < MEASUREMENT_COUNT)) && (timeout < 2000)) begin
            @(posedge clk);
            timeout = timeout + 1;
        end
        ...
        if (envelope_index != ENVELOPE_COUNT ||
            ...
            $fatal(1, "M6 result counts E/M=%0d/%0d",
                   envelope_index, measurement_index);
        end
        ...
        $fclose(result_file);
        $display("M6_SIGNAL_PROCESSING_SIM_PASS envelope=%0d measurement=%0d",
                 envelope_index, measurement_index);
        $finish;
    end
