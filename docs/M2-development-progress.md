...
> 开始日期：2026-07-11  
> 当前状态：RTL、仿真、实现、下载、双通道环回、相位扫描及 M1/M2 联合实机验收均已完成
...
- [x] 与 M1 一起完成示波器及 DAC→ADC 环回实机验证。
- [x] 删除旧的 M0/M1/M2 ILA 和 M1/M2 VIO，释放系统资源。
...
- 相位扫描最终选择 401 步，即约 `5.508ns`；删除 VIO 后将其固化为上电默认读相位。
- 验收阶段使用 VIO 每次移动 56 步，即约 `0.769ns / 18°`；65MHz 一个完整周期为 1120 步。
- M1/M2 验收期间保留 DAC、VIO 和 ILA；验收完成后统一删除调试核。
- 后续回归按需临时插入专用 ILA，不累积保留旧阶段调试核。
...
- 删除顶层 `u_ila_m0/u_ila_m1/u_ila_m2/u_vio_m2`，并从 Vivado 工程删除 `ila_m0/ila_m1/ila_m2/vio_m1/vio_m2` IP。
- 删除仅为 ILA 服务的提交计数、相位 Gray CDC 和探针连线；ADC 时钟存活检测改为单比特安全 CDC。
- ADC 读相位固化为相位扫描选定的 401 步，即约 5.508ns。
- 清理后回归仿真、综合、实现、比特流生成和 FPGA 下载均通过；Hardware Manager 确认设计中没有受支持的软调试核。
...
| M1/M2 联合实机验证 | 通过 | DAC→ADC 双通道环回、相位扫描及 M1 双通道示波器验收均已完成 |

清理调试核后的最终全工程资源：1594 LUT、1897 FF、2 BRAM Tile、4 DSP、1 MMCM、5 BUFG。
