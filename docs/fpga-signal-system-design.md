...
> 上位机：Python + PyQt + SQLite，运行于 Windows
...
4. **上位机软件**：PyQt 界面 + UART 控制 + 以太网数据接收 + SQLite 存储，实现波形显示、参数控制、测量和数据回放。
...
| 板上处理 | **抽取、Min/Max、均值、峰峰值、周期测量** | 显著降低网络带宽和上位机负担；RMS 延后到后续阶段 |
| 参数控制入口 | **全部由上位机下发** | 上位机为唯一控制入口，FPGA 无需本地按键 |
| 触发方式 | **边沿触发**，支持阈值、迟滞、上升/下降沿和预触发 | 提高噪声环境下的触发稳定性 |
| 数据持久化 | **SQLite 存储记录、参数及完整帧 BLOB** | 单机单用户无需数据库服务，单文件便于备份和回放 |
...
按 32bit 对齐计算，256MiB DDR3 理论上可保存约 1.03 秒双通道 65Msps 数据。M6 固化地址划分后，前 224MiB 保存 RAW32 环形帧（最大 `58,720,256` 点，约 0.90 秒），后 32MiB 保存 DECIMATED32 抽取帧（最大 `8,388,608` 点）。
...
| 数据路径 | 内容 | 用途 |
|---|---|---|
| `RAW_FRAME` | DDR3 中的 32bit 原始样本组 | 精确分析、存储 |
| `ENVELOPE_FRAME` | 每个时间桶的 A/B 最小值和最大值 | 示波器式连续显示、保留窄脉冲和峰值 |
| `DECIMATED_FRAME` | 三级 CIC 低通后的 A/B 32bit 样本组 | 低频长时间记录 |
| `MEASUREMENT` | Min、Max、均值、Vpp、周期/频率、OTR 计数 | 快速读数和状态显示 |

- **FR-ACQ-10** 低频长时间记录采用三级 CIC 数字低通和可编程抽取后写入 DDR3，M6 只允许 `1/2/4/.../1024` 倍率；禁止仅丢点而不进行抗混叠处理。
- **FR-ACQ-11** “连续采集”特指 `ENVELOPE_FRAME` 或抽取数据连续上传，不表示双通道 65Msps 原始数据无损连续上传。
- **FR-ACQ-12** 原始触发帧下载期间，DDR3 中的帧必须保持到上位机确认完成。
...
- **FR-NET-8** 固定部署使用 FPGA `192.168.1.10`、PC `192.168.1.100/24` 和 UDP `5001`；不占用 NI Time Synchronization 使用的 UDP `5000`。

### 3.4 上位机软件（Python / PyQt / SQLite）
...
- **FR-PC-6** 数据存储：采集记录、配置、测量结果和完整帧以 SQLite 记录及 BLOB 形式保存，支持查询和回放。
- **FR-PC-7** 界面分区：连接状态、发生控制、采集控制、波形显示、测量读数和记录管理。
- **FR-PC-8** 支持一键启动：首次运行持久配置 PC 辅助 IP，并仅为 FPGA 地址放行 UDP `5001` 入站流量；后续自动启动 UDP，并优先连接上次成功的串口；只有一个串口且无历史配置时自动连接该串口。
...
4. **DDR3 帧管理**：RAW 区为 `0x00000000..0x0DFFFFFF`，抽取区为 `0x0E000000..0x0FFFFFFF`（MIG 地址分别为 `0` 和 `28'h07000000`）；描述符保存起始地址、总点数、触发索引、有效采样率、通道掩码、抽取倍率和状态标志。
...
| 上位机 | PyQt + SQLite | UART 控制、UDP 数据、显示、存储和回放 |

### 5.1 能力边界

- AD9226 单片最高 65Msps；只有“单通道 100Msps 采样率”需要 130Msps 交织。第一阶段 65Msps 已满足单通道 4MHz 和双通道 2MHz 带宽。
- M6 的 224MiB RAW 区按 32bit 样本组可保存约 0.90 秒满速双通道数据；32MiB 抽取区在最大 1024 倍抽取时可保存约 132 秒数据，更长低频时间窗仍需分段。
...
- **NFR-6** 上位机使用 Python 3、PyQt5、pyserial、pyqtgraph、numpy 和标准库 `sqlite3`；UDP 使用 Python 标准库 `socket`。
- **NFR-7** SQLite 使用单文件数据库和 WAL 模式；完整帧以单条 BLOB 存储，禁止逐样本逐行插入数据库。
...
| SQLite 表结构 | M8 定义 | 使用采集记录、测量结果和帧 BLOB 三类表，单文件保存 |
