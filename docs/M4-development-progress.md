...
> 开始日期：2026-07-14  
> 当前状态：RTL/IP、仿真、实现与 bitstream 已完成，待上板校准和长时间压力测试  
> 板级事实来源：`references/Davinci_Lite_1V0.pdf`  
> DDR3 时序来源：南亚 `NT5CC128M16JR-EK` 准确料号数据手册

## 1. 目标与验收标准

- [ ] Vivado MIG 使用板载 `NT5CC128M16JR-EK`，完成 DDR3L 初始化与校准。
...
- [ ] 多次上电均完成 MIG 校准，持续数分钟压力测试无错误。
- [x] 行为模型下持续写有效吞吐量高于 260MB/s；板级实测待完成。
...
- [x] 建立 `run_m4_build.tcl`，自动执行综合、实现、bitstream 和签核报告生成。
...
- [x] 最终时序满足全部约束：WNS=`+0.530ns`、WHS=`+0.047ns`、TNS/THS=`0`。
- [x] 路由检查：17635 个 fully routed nets，routing errors=`0`。
...
- [x] 实现后资源复核：LUT 44.01%、寄存器 24.89%、BRAM 28%、DSP 4.44%、MMCM 3/5、PLL 1/5。
...
- [ ] 下载 `Signal.runs/impl_1/Top.bit`，多次冷启动确认 `init_calib_complete=1`。
- [ ] UART 状态查询确认 `DDR_CALIBRATED=1`。
- [ ] ILA 确认五种模式、起始/中间/末尾/全空间循环持续数分钟，`error_count=0`。
- [ ] 记录板级校准周期和峰值/持续读写吞吐量，确认持续写高于 260MB/s。
